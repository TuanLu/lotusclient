...
class EditableTable extends React.Component {
  ...
  save(form, key) {
    form.validateFields((error, row) => {
      ...
      if (index > -1) {
        ...
        fetch(ISD_BASE_URL + 'agency/updateAgency', {
          method: 'POST',
  ...
  delete = (record) => {
    if(record.id) {
      fetch(ISD_BASE_URL + 'agency/deleteAgency/' + record.id, {
        headers: getTokenHeader()
      })
      .then((response) => response.json())
      .then((json) => {
        if(json.status == 'error') {
          message.error('Có lỗi xảy ra khi xóa nhà phân phối!', 3);
        } else {
  ...
  fetchData() {
    fetch(ISD_BASE_URL + 'agency/fetchAgency', {
      headers: getTokenHeader()
    })
    .then((response) => {
      return response.json();
            ...
            <Col span={12}>
              <div className="action-btns">
                <Button 
                  ...
                  type="primary" icon="plus">Thêm Nhà Phân Phối</Button>
              </div>
            </Col>
          </Row>
        </div>
