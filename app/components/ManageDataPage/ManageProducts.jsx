...
class EditableTable extends React.Component {
  ...
  save(form, key) {
    form.validateFields((error, row) => {
      ...
      if (index > -1) {
        ...
        fetch(ISD_BASE_URL + 'product/updateProduct', {
          method: 'POST',
  ...
  delete = (record) => {
    if(record.id) {
      fetch(ISD_BASE_URL + 'product/deleteProduct/' + record.id, {
        headers: getTokenHeader()
      })
  ...
  fetchData() {
    fetch(ISD_BASE_URL + 'product/fetchProducts', {
      headers: getTokenHeader()
    })
    .then((response) => {
      return response.json();
    ...
    return (
      <Layout>
        <div className="table-operations">
          <Row>
            <Col span={12}>
              <h2 className="head-title">Quản lý sản phẩm</h2>
            </Col>
            <Col span={12}>
              <div className="action-btns">
                <Button 
                  onClick={() => this.addNewRow()}
                  type="primary" icon="plus">Thêm Sản Phẩm</Button>
              </div>
            </Col>
          </Row>
        </div>
